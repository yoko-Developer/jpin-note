...
前後関係を全部読まなければ影響範囲の特定が困難

✨別の場所にこれらのデータを必要とする処理がないようにする

✨関係する単位にデータを纏める: 名前、製造年月日、消費期限

✨過不足なくデータを作る(無関係のデータを入れてはダメ)

✨変更の際どこに影響があるかすぐにわかることが必須
...
グループを作ることが必要
1. が前提(変更の影響範囲をすぐに特定できる)

### Outside-in

- カプセル化しない(カプセル化を壊してしまう可能性があるため)
- フィールドなし、getter,setterによってデータがどんどん運ばれていくことになる
- データと処理が別々になりがちなので、処理に依存するデータのルールを変更すると他の処理に影響が及ぶため特定しづらくなる
...
外部からはオブジェクト内部のデータにアクセスできないようにする

アクセサメソッド、getter/setterは使用禁止
